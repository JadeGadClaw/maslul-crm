...
function israelDateKey(date = new Date()) { return israelDateFormatter.format(date); }
function israelMonthKey(date = new Date()) { return israelMonthFormatter.format(date); }
function israelYearKey(date = new Date()) { return israelYearFormatter.format(date); }
...
export default function Page() {
  ...
  function saveJob() {
    ...
    setJobs((current) => current.some((job) => job.id === next.id) ? current.map((job) => job.id === next.id ? next : job) : [next, ...current]); setSelectedId(next.id); setDraft(emptyJob());
  }
