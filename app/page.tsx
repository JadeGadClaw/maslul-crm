...
function sameDay(date: string, target = new Date()) { return date === target.toISOString().slice(0, 10); }
...
function Summary({ jobs }: { jobs: Job[] }) {
  ...
  const thisMonth = jobs.filter((j) => monthKey(j.date) === now.toISOString().slice(0, 7));
  const thisYear = jobs.filter((j) => yearKey(j.date) === String(now.getFullYear()));
  ...
  const topRoutes = Object.entries(jobs.reduce<Record<string, number>>((acc, job) => { const key = `${job.from} ← ${job.to}`; acc[key] = (acc[key] || 0) + 1; return acc; }, {})).sort((a, b) => b[1] - a[1]).slice(0, 4);
...
function Receipt({ job }: { job: Job | null }) {
  ...
  return <section className="panel receipt" id="receipt-template">
    ...
    <dl><dt>מספר נסיעה</dt><dd dir="ltr">{job.id}</dd><dt>תאריך ושעה</dt><dd>{formatDate(job.date)} · {job.time}</dd><dt>לכבוד</dt><dd>{job.client || '—'} / {job.company || '—'}</dd><dt>מסלול</dt><dd>{job.from} ← {job.to}</dd><dt>רכב / עבודה</dt><dd>{vehicleLabels[job.vehicleType]} · {job.workType}</dd><dt>נהג</dt><dd>{job.driver || '—'}</dd><dt>סטטוס תשלום</dt><dd>{paymentLabels[job.paymentStatus]}</dd><dt>הערות</dt><dd>{job.notes || '—'}</dd></dl>
...
export default function Page() {
  ...
  useEffect(() => { setAuthed(localStorage.getItem(authKey) === 'ok'); const saved = localStorage.getItem(storageKey); setJobs(saved ? JSON.parse(saved) : seedJobs); }, []);
  ...
  const filteredJobs = useMemo(() => jobs.filter((job) => {
    const haystack = `${job.client} ${job.company} ${job.from} ${job.to} ${job.notes} ${job.workType}`.toLowerCase();
  ...
  function login(e: React.FormEvent) { e.preventDefault(); if (pin === demoPin) { localStorage.setItem(authKey, 'ok'); setAuthed(true); } }
  function saveJob() { const next = { ...draft, updatedAt: new Date().toISOString(), price: Number(draft.price) || 0 }; setJobs((current) => current.some((job) => job.id === next.id) ? current.map((job) => job.id === next.id ? next : job) : [next, ...current]); setSelectedId(next.id); setDraft(emptyJob()); }
  ...
  function exportCsv() {
    const columns: Array<[keyof Job, string, (job: Job) => unknown]> = [['id', 'מספר נסיעה', (j) => j.id], ['date', 'תאריך', (j) => j.date], ['time', 'שעה', (j) => j.time], ['client', 'שם לקוח', (j) => j.client], ['company', 'חברה', (j) => j.company], ['from', 'איסוף', (j) => j.from], ['to', 'יעד', (j) => j.to], ['driver', 'נהג', (j) => j.driver], ['vehicleType', 'רכב', (j) => vehicleLabels[j.vehicleType]], ['price', 'מחיר', (j) => j.price], ['paymentStatus', 'סטטוס תשלום', (j) => paymentLabels[j.paymentStatus]], ['receiptStatus', 'סטטוס קבלה', (j) => receiptLabels[j.receiptStatus]], ['notes', 'הערות', (j) => j.notes]];
    ...
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); const a = document.createElement('a'); a.href = url; a.download = `נסיעות-${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
  }

  if (!authed) return <main className="login"><form onSubmit={login} className="panel login-card"><p className="eyebrow">מסלול CRM — דמו</p><h1>מסלול CRM</h1><p className="muted">כניסת מנהל מקומית לדמו. קוד כניסה: <code dir="ltr">admin123</code></p><input type="password" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="קוד מנהל" autoFocus /><button className="primary">כניסה ליומן</button></form></main>;

  return <main className="shell">
    <header className="hero"><div><p className="eyebrow">מסלול CRM</p><h1>נסיעות, קבלות והכנסות ביומן אחד.</h1><p className="muted">גרסת דמו מקומית להחלפת גיליון העבודה הקיים. הנתונים נשמרים בדפדפן עד לחיבור מסד נתונים אמיתי.</p></div><div className="actions"><button onClick={exportCsv}>ייצוא נתונים</button><button onClick={resetData}>איפוס נתוני דמו</button><button onClick={() => { localStorage.removeItem(authKey); setAuthed(false); }}>יציאה</button></div></header>
    <Summary jobs={jobs} />
    ...
    <section className="panel jobs-panel"><div className="section-head"><div><p className="eyebrow">יומן נסיעות</p><h2>{filteredJobs.length} נסיעות</h2></div></div>
